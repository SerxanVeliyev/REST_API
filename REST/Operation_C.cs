...
namespace REST
{
    public class Operation_C
    {
        ...
        public string Operation(int page)
        {
            ...
            SQL_Com = new SqlCommand(@"WITH OPERATION (ID,IL,AY,GUN,CREATEDDATE,OPERATORNAME,EXPEDİTORNAME,NAME,CONFIRM) AS(
                                        SELECT ORD.Id,
	                                        ...
	                                        MONTH(ORD.CREATEDDATE) AS [AY] ,
	                                        DAY(ORD.CREATEDDATE) AS [GÜN] ,
	                                        CONVERT(nvarchar, ORD.CREATEDDATE, 20) ,    
	                                        USR.FULLNAME OPERATORNAME,
	                                        USRDRV.FULLNAME EXPEDİTORNAME,
	                                        İTM.NAME,
	                                        (CASE 
                                        ...
                                        SELECT TOP(100) ID AS [id],IL AS [year],AY AS [month],GUN AS [day],CREATEDDATE AS [createDate],OPERATORNAME AS [operatorName] ,EXPEDİTORNAME AS [expeditorName],NAME AS [name],CONFIRM AS [confirm] FROM OPERATION");
